// RecorderCommand.cpp : 定义控制台应用程序的入口点。
//
...
#pragma comment(lib,"../../ffmpeg3.4.2/lib/swresample.lib");
    ...
    int qwemain(int argc, char **argv)
    {
     const char *filename;
     ...
     //input_file = "video=WebCam SC-10HDD12636N";
     filename = "commandSSSS.mp4";
	input_file = "testSrc.mov";
	ifmt = NULL;
     if (avformat_open_input( &input_fmt_ctx, input_file, ifmt, NULL) < 0) {
     ...
     if (got_frame) {
		 ...
		 struct SwsContext *img_convert_ctx = sws_getContext(video_dec_ctx->width, video_dec_ctx->height, video_dec_ctx->pix_fmt,video_dec_ctx->width, video_dec_ctx->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);;
		 sws_scale(img_convert_ctx, (const unsigned char* const*)input_frame->data, input_frame->linesize, 0, input_frame->height,pFrameYUV->data, pFrameYUV->linesize);	


	//avpicture_fill(&dst_picture,(const uint8_t *)input_frame->data,video_dec_ctx->pix_fmt,video_dec_ctx->width,video_dec_ctx->height);
     av_image_copy(&dst_picture.data[0], video_dst_linesize,
